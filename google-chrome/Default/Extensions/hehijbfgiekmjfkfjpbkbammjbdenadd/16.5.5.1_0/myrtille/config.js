    ...
    vmNotEnhanced)                                      // RDP over VM bus (Hyper-V), simple session mode
{
    ...
    // dialog
    var debugConsole = true;                           // output the debug messages into the browser javascript console (or on screen otherwise)
    ...
    var imageQuality = 65;                             // image quality (%) higher = better; not applicable for PNG (lossless); tweaked dynamically to fit the available bandwidth if using JPEG, AUTO or WEBP encoding. for best user experience, fullscreen updates are always done in higher quality (75%), regardless of this setting and bandwidth
    ...
    var adaptiveFullscreenTimeout = 500;               // adaptive fullscreen update (ms); requested after a given period of user inactivity (=no input). 0 to disable
    ...
    var networkMode = networkModeEnum.WEBSOCKET;             // network mode
    ...
    var bufferEnabled = false;                           // buffer for user inputs; adjusted dynamically to fit the latency
