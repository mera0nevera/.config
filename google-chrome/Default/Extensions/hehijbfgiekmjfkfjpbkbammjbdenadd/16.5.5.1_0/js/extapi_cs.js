...
var ExtensionApi = {
            ...
            elExtra.addEventListener('click', function() {
                chrome.runtime.sendMessage({ type: 'REQUEST_PERMISSIONS' }, function(permissions) {
                    if (permissions && permissions.downloads) {
                        alert('Thanks!  IE Tab will now perform early interception for all downloaded files');
    ...
    init: function() {
        ...
        var url = chrome.runtime.getURL('js/extapi_wp.js');
        script.src = url;
        ...
        window.addEventListener('message', function(event) {
            ...
            if (event.data.type == 'EXTAPI_REQUEST') {
                ...
                if (fnResponse)
                    chrome.runtime.sendMessage(event.data, fnResponse);
                else
